...
if get(hObject,'Value') == 2 && ~handles.roson
    try
        handles.operando = false;
...
% --- Executes on selection change in Conexion.
function Canoxion_Callback(hObject, eventdata, handles)
% hObject    handle to Canoxion (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Canoxion as text
%        str2double(get(hObject,'String')) returns contents of Canoxion as a double
handles = guidata(hObject);
handles.hostip = get(hObject,'String');
disp(handles.hostip)
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function Canoxion_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Canoxion (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
